...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView sd_setImageWithURL:self.imagesGroup[itemIndex] placeholderImage:[UIImage imageNamed:@"placeholder_banner"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (error) {
            cell.imageView.image = [UIImage imageNamed:@"failed_banner"];
        }
    }];
...
{
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectBannerAtIndex:)]) {
        [self.delegate cycleScrollView:self didSelectBannerAtIndex:indexPath.item % self.imagesGroup.count];
    }
}
